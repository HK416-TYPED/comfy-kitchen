...
// Kitchen integration shims: upstream nunchaku depends on spdlog and
// <cublas_v2.h>, neither of which is on kitchen's include path. We replace
// them with inline stubs so the zgemm code compiles unchanged inside kitchen.
// - spdlog::{trace,debug,info,warn,error,critical}(fmt, args...) -> no-op
// - spdlog::fmt_lib::format(...)                                 -> std::format
// - cublas_v2.h + checkCUBLAS()                                  -> unused on
//   the W4A4 path, so the cuBLAS include is dropped and the helper is omitted.
namespace spdlog {
    template<typename... Args> inline void trace(Args&&...) noexcept {}
    template<typename... Args> inline void debug(Args&&...) noexcept {}
    template<typename... Args> inline void info(Args&&...) noexcept {}
    template<typename... Args> inline void warn(Args&&...) noexcept {}
    template<typename... Args> inline void error(Args&&...) noexcept {}
    template<typename... Args> inline void critical(Args&&...) noexcept {}
}
...
// CUBLASWrapper / getCUBLAS() removed — cuBLAS is not used on the W4A4 path
// and kitchen doesn't depend on cublas_v2.h. See header comments above.
